    ...
    private fun registerEventView() {
        btnAudioVideoCAllActivity.setOnClickListener {
            CoroutineScope(Dispatchers.Main).launch {
                ...
                if(isSpeaker){
                    btnAudioVideoCAllActivity.setImageDrawable(ContextCompat.getDrawable(this@VideoCallActivity,R.drawable.outspeaker))
                }else{
                    btnAudioVideoCAllActivity.setImageDrawable(ContextCompat.getDrawable(this@VideoCallActivity,R.drawable.headphone))
                }
            }
        }
...
//        }
    ...
    private fun initCall() {
        if (isComingCall == true) {
            call= HomeActivity.videoCallMap[callid]
            if (call==null) {
                finish()
                return
            }
            ...
            call = StringeeCall2(HomeActivity.client, HomeActivity.client?.userId, to)
            layoutHandleInCallingVideoCAllActivity.visibility= View.GONE
            layoutVideoCAllActivityMain.visibility= View.VISIBLE
        }
